...
function update() {
  ...
  progress.value = total > 0 ? Math.min(scrollY / total, 1) : 0
}

const onScroll = throttle(update, 50)
...
<template>
  <Transition name="btt-fade">
    <div v-show="visible" class="back-to-top" title="返回顶部" @click="scrollToTop">
      <svg class="btt-ring" viewBox="0 0 44 44">
        <circle
